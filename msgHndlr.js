...
module.exports = msgHandler = async (client, message) => {
    try {
        ...
        case '!donasi':
        case '!donate':
            client.sendLinkWithAutoPreview(from, 'no', donate)
            break
        case '!tts':
        ...
        case '!bc':
            ...
            for (let ids of chatz) {
                ...
                if (!cvk.isReadOnly) await client.sendText(ids, `[ A B I Y U B O T ]\n\n${msg}`)
            }
        ...
        case '!leave':
            if (!isGroupMsg) return client.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isGroupAdmins) return client.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin group', id)
            await client.sendText(from,'DADAH:(').then(() => client.leaveGroup(groupId))
            break
        case '!promote':
            if (!isGroupMsg) return client.reply(from, 'Fitur ini hanya bisa di gunakan dalam group', id)
        ...
        case '!demote':
            ...
            break
        case '!join':
            //return client.reply(from, 'Jika ingin meng-invite bot ke group anda, silahkan izin ke wa.me/6281386603076', id)
        ...
        case '!meme':
            ...
            break
        case '!menubot':
            client.sendText(from, menubot, MenuBot, help)
            break
        case '!readme':
